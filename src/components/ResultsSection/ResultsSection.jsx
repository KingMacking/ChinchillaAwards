			...
			const sortedParticipants = Object.entries(categoryVotes)
				...
				.map(([participantId, points]) => {
					...
					return {
						...
						points,
						name:
							selectedTab === "categories" ? participant.name : participant.clipName,
						img:
							selectedTab === "categories"
								? `/assets/participants-pictures/${participant.image}`
								: null,
					};
				});
		...
		<div className='w-full max-w-4xl p-4 mx-auto text-white bg-[#000816] bg-opacity-50 rounded-lg shadow-md backdrop-blur-md gap-4 flex items-start'>
			...
			<div className='flex flex-col gap-1 min-w-64'>
				...
				<button
					type='button'
					onClick={goBack}
					className='px-4 py-2 text-lg font-bold text-white uppercase transition-all duration-200 border border-transparent rounded-lg cursor-pointer category-item bg-primary'
				>
					Volver atrás
				</button>
			</div>
			...
			<div className='flex-[1]'>
				{selectedCategory && (
					<div className='result-details'>
						{(() => {
							// Calcular el total de puntos de todos los resultados
							const totalPoints = currentResult?.results.reduce(
											...
											>
												<div className='flex items-center gap-6'>
													...
													<div>
														...
														<p className='text-white'>
															Puntos:{" "}
															{currentResult?.results[0].points}{" "}
															{totalPoints > 0 && (
																<span>
																	(
																	{(
